...
- 영상 수신 파트에서 여러가지 가정을 두고 관련 코드 사이에 로그메시지를 체크하면서 원인을 파악.
  - 가정1: 4개의 화면 중 하나 이상의 영상이 수신되지 않을때 문제 발생 => 4개의 영상을 모두 수신이 되도록 설정하고 테스트하였으나 문제 해결되지 않음.
  - 가정2: 스레드 영상을 Join하는 것이 시간이 오래 걸려 그 시간동안 UI 스레드가 오랫동안 블로킹되어 문제 발생 => Join 코드를 주석처리하고 재실행 하였으나 문제 해결되지 않음.  
  - __가정3__: 영상화면을 destroy한 후 비트맵에 이미지를 그리는 과정이 오래 걸려 그 시간동안 UI 스레드가 오랫동안 블로킹되어 문제 발생 => 원인 파악완료  
      ```java
