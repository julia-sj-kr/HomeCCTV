...
음성 인식 애플리케이션은 `SpeechRecognizer 객체 사용'과 '음성 인식 인텐트 필터 사용' 2가지 방식을 알아보았습니다.  
그 중 음성 인식 인텐트 필터를 사용한 애플리케이션의 음성 인식 정확도가 높고 속도가 빨라서 홈CCTV 프로그램에는 해당 기술을 사용하였습니다.  

a. Android 플랫폼에서 RecognizerIntent를 사용하여 시스템 음성 인식 기능을 호출함.   
b. UDP 프로토콜을 통해 인식된 음성을 네트워크로 전송합니다.  
...
Task 05에서 음성으로 화면 움직인 제어 하는 기능은 '약속된 명령어'일때만 작동하는 프로세스입니다.  
여기에 챗 GPT API를 연결하여 '사용자 명령'이 무엇이든지 뜻을 파악하여 약속된 명령어로 변환해주는 기능을 추가하였습니다.
